...
export default function Signup() {
  const r = useRouter();
  const [f, setF] = useState({ name: "", brand: "", email: "", password: "" });
  const [err, setErr] = useState("");
  const submit = async () => {
    const res = await fetch("/api/auth/signup", { method: "POST", body: JSON.stringify(f) });
    if (res.ok) r.push("/onboarding"); else setErr((await res.json()).error || "Error");
  };
  return (
    <div className="min-h-screen flex items-center justify-center px-6">
      <div className="bg-zinc-900/70 border border-zinc-800 rounded-2xl p-8 w-full max-w-md">
        ...
        <div className="space-y-3 mt-6">
          {["name:Your name", "brand:Brand name", "email:Email", "password:Password"].map((s) => {
            const [k, ph] = s.split(":");
