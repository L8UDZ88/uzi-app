...
export async function POST(req: Request) {
  ...
  const { campaignId, mood, seconds } = await req.json();
  ...
  const prompt = `Instrumental background score for a premium social ad${region}. Mood: ${mood || voice}. Clean, modern, continuous, leaves room for a voiceover on top. High-quality production. Strictly instrumental — no vocals, no singing, no spoken word.`;
  const r = await generateMusic(prompt, Number(seconds) || 30);
